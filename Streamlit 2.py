...
economy_selected = st.checkbox("Economy", value=True)
business_selected = st.checkbox("Business", value=False)
...
# Filter data berdasarkan input pengguna dan kelas yang dipilih
filtered_df = df[(df['source_city'] == source_city_encoded) & (df['destination_city'] == destination_city_encoded)]

# Tambahkan filter untuk kelas jika dipilih
if not economy_selected and not business_selected:
    st.write("Please select at least one class.")
else:
    if not economy_selected:
        filtered_df = filtered_df[filtered_df['class'] == 1]  # Hanya Business
    elif not business_selected:
        filtered_df = filtered_df[filtered_df['class'] == 0]  # Hanya Economy
    ...
    def generate_combinations(filtered_df, days_left, sample_size=10):
        ...
        classes = [0, 1] if (economy_selected and business_selected) else [0] if economy_selected else [1]
    ...
    if st.button("Predict"):
        ...
        # st.write("Top 5 Cheapest Prices:")
        # st.write(decoded_data.head(5))
